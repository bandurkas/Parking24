...
export default async function PolicyPage() {
  ...
  const fields = [
    { key: "route" as const, label: LINKS.route.label, hint: p.routeFallback ? `Строка «Маршрут:» в сообщениях. Если пусто, подставится ${p.routeFallback}` : "Строка «Маршрут:» в сообщениях. Если пусто, строка выпадет" },
    { key: "review" as const, label: LINKS.review.label, hint: "Переменная {{links.review}}. Если пусто, строка с ней выпадет из сообщения" },
    { key: "video" as const, label: LINKS.video.label, hint: "Переменная {{links.video}} — видео ждём от заказчика. Если пусто, строка с ней выпадет" },
  ];
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {p.pendingOutbox > 0 && (
        <p className="mt-2 text-sm text-ink-muted">
          В очереди {p.pendingOutbox} неотправленных сообщений — они уже собраны со старыми ссылками, новая ссылка попадёт только в следующие.
        </p>
      )}
