// Сводка по людям без денег: смены по слотам, всего, часы из записи смены
...
export default function MonthSummary({ rows, title = "Сводка за месяц" }: { rows: ReportRow[]; title?: string }) {
  const total = rows.reduce((a, r) => ({ total: a.total + r.total, hours: a.hours + r.hours }), { total: 0, hours: 0 });
  return (
    <section className="adm-card mt-4 overflow-x-auto" aria-label={title}>
      <table className="w-full text-sm" data-summary>
        ...
        <tbody className="tnum">
          {rows.length === 0 && (
            <tr>
              <td colSpan={7} className="px-3 py-3 text-ink-muted">Смен нет</td>
            </tr>
          )}
          {rows.map((r) => (
