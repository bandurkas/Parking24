...
export default function TemplateCard({ t, vars }: { t: TemplateView; vars: { name: string; sample: string }[] }) {
  ...
  function restore() {
    setNote(null);
    start(async () => {
      const res = await restoreTemplateAction(t.code);
      setAskRestore(false);
      setNote(res.ok ? { ok: true, text: "Возвращён текст поставки" } : { ok: false, text: res.error });
      if (res.ok) {
        setPreview(null);
        router.refresh();
      }
  ...
  return (
    <section className="adm-card p-4 sm:p-5" data-template={t.code} aria-label={`Шаблон ${t.name}`}>
      <div className="flex flex-wrap items-start gap-2">
        <div className="min-w-0 flex-1">
          <input
            ...
            className="w-full rounded-md bg-transparent text-base font-semibold outline-none focus:bg-surface-soft focus:px-2"
          />
          <div className="font-mono text-[11px] text-ink-muted">{t.code}</div>
        </div>
        {t.edited ? (
          ...
          <span data-testid="default-badge" className="rounded-md bg-surface px-2 py-1 text-xs font-semibold text-ink-muted">Текст поставки</span>
        )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {dirty && (
          <button type="button" onClick={() => { setName(t.name); setBody(t.body); setWarn(null); }} className="adm-btn-ghost h-10 px-3 text-sm">
            Отменить правку
          </button>
        )}
      ...
      {preview !== null && (
        <div className="mt-3 rounded-lg bg-surface-soft p-3" aria-label="Предпросмотр">
          ...
          <div data-testid="preview" className="whitespace-pre-wrap text-sm">{preview}</div>
        </div>
      )}
      ...
      {t.edited && (
        t.defaultBody !== null ? (
          <details className="mt-3 text-sm">
            <summary className="cursor-pointer text-ink-muted">Текст поставки</summary>
            <div className="mt-2 whitespace-pre-wrap rounded-lg bg-surface-soft p-3 text-ink-muted">{t.defaultBody}</div>
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {askRestore ? (
                <>
                  <span className="text-ink-muted">Ваш текст заменится текстом поставки.</span>
                ...
                <button type="button" onClick={() => setAskRestore(true)} className="adm-btn h-9 px-3 text-sm">
                  <RotateCcw size={14} /> Вернуть текст поставки
                </button>
              )}
            </div>
          </details>
        ) : (
          <div className="mt-3 text-xs text-ink-muted">Текст поставки не записан</div>
        )
      )}

      <div className="mt-3 border-t border-line pt-3 text-sm">
        <div className="text-xs font-semibold uppercase tracking-wide text-ink-muted">Когда уходит</div>
        {t.rules.length === 0 ? (
      ...
      <details className="mt-3 text-sm">
        ...
        <div className="mt-2 grid gap-x-4 gap-y-1 sm:grid-cols-2">
          {vars.map((v) => (
            <div key={v.name} className="min-w-0 truncate" title={v.sample}>
              <span className="font-mono text-xs text-primary-deep">{`{{${v.name}}}`}</span>
              <span className="text-ink-muted"> — {v.sample || "пусто (строка выпадет)"}</span>
            </div>
          ))}
        </div>
      </details>
