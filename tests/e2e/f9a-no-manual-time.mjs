...
// correction.test.ts («заезд и выезд в один прошедший день»); отказ сервера «Укажите дату выезда» — форма шлёт пустую строку,
// и сервер отказывает раньше, на формате (проверяется сам отказ); отказ сервера администратору из закрытого статуса мимо
...
try {
  ...
  // ── Н7. «Ожидает оплаты» → «Выехал» спрашивает обе даты (§10.9, e2e №7) ──
  {
    ...
    check("Н7: без даты выезда в обход формы — отказ сервера", /Укажите дату выезда|Дата в формате ГГГГ-ММ-ДД/.test(noOut), noOut || "ошибки нет");
    await fieldOut(form).evaluate((el) => el.removeAttribute("min"));
    await fillReliably(fieldOut(form), yesterday);
