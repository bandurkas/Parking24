...
try {
  ...
  // ── Н1. Карточка: «Ожидает оплаты» → «Заехал» → «Выехал» одним нажатием, время серверное (§10.1, e2e №1) ──
  {
    ...
    // Ф10: сутки считаются по датам нового выезда (досрочный выезд → баннер без часов), а не по снятому
    const b1b = await body(P);
    check("Н1: баннер по датам нового выезда — «Стоянка по факту: 1 сут. (по плану 3)», без часов", /Стоянка по факту: 1 сут\. \(по плану 3\)/.test(b1b), b1b.match(/Стоянка по факту[^\n]*/)?.[0] ?? "баннера нет");
  }
  ...
  // ── Н4. «Исправить статус» датой: отказы по датам, успех «Заехал» и «Выехал», лента и журнал (§10.4–10.6, §10.8, e2e №3–5) ──
  {
    ...
    check("Н4: баннер по датам (Ф10) — «Стоянка по факту: 1 сут. (по плану 3)»", /Стоянка по факту: 1 сут\. \(по плану 3\)/.test(b4), b4.match(/Стоянка по факту[^\n]*/)?.[0] ?? "баннера нет");
  ...
  // ── Н8. Перестой: confirm с суммой в карточке и на «Сегодня», отказ ничего не меняет; исправление не начисляет (§12 в.1, e2e №6) ──
  {
    ...
    check("Н8: баннера «Стоянка по факту» нет — исправление датой доплату не предлагает (Ф10)", !/Стоянка по факту/.test(await body(P)));
  ...
  // ── Н11. Перестой, заезд не отмечен: «Подтверждена» → «Выехал» двумя датами тоже не начисляет и пишет это в ленту ──
  {
    ...
    check("Н11: баннера «Стоянка по факту» нет — исправление датой доплату не предлагает (Ф10)", !/Стоянка по факту/.test(await body(P)));
