...
// опечатка в переменной, предупреждение о номере брони, seed не затирает правку, «Вернуть текст по умолчанию»; выключатель правила
...
try {
  ...
  if (!CODE) console.log("  – раздел шаблонов пропущен: все шаблоны с номером брони правлены вручную, чужой текст не трогаем");
    ...
    savedByRun = true; // до клика: сохранение могло пройти, даже если ожидание бейджа упадёт
