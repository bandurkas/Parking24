...
> **Состояние на 22.09, день: блокеры закрыты, два подтверждающих раунда пройдены, второй — чистый.**
>
> | Коммит | Что | Подтверждающее ревью |
> |---|---|---|
> | `b2e3ed7` | 4 блокера + находки `bookings/[id]:47`, `clients/[id]:163`, `render.ts:41` (суммы «1 050»), мелочи `BookingsTable:12`, `TransitionButtons:82`, `dispatcher.ts:28` и `settings.ts:50` (siteLinks подключена) | раунд 1: блокеры закрыты, но 2 важных — повтор «место забронировано» при оплате, «0 ₽» у фуры |
> | `baaf46c` | обе важные + правило пустых подписей не режет строку по точке + отменённая запись очереди не держит ключ | раунд 2: **ЧИСТО**, 4 мелочи |
>
> Находка ⏳ `settings.ts:57` (siteLinks и `NEXT_PUBLIC_SITE_URL` на stage) **опровергнута**: если переменной нет при сборке, Next в серверном коде её не вшивает и читает во время работы — проверено по собранному бандлу на stage, значение в серверном `.env` есть.
>
> **Открытые мелочи раунда 2** (не блокируют выкатку):
> 1. На stage записи очереди, поставленные до выкатки, держат старые ключи `on_awaiting_payment:<id>` — после оплаты такой брони будет второе подтверждение. Отправки нет, вреда нет; до включения отправщика переименовать ключи в `confirmation:<id>` или отменить устаревшие (план этапа 2, «Безопасность выкатки»).
> 2. Фура с ценой 0, оплаченная без галочки «Это полная стоимость», сама переходит в «Подтверждена», а в сообщении «Стоимость подскажет администратор» без слова «оплачено». Правка: при цене 0 не подтверждать бронь оплатой автоматически или писать «Оплачено: N ₽».
> 3. Оживление отменённой записи очереди проверено ревьюером на имитации, e2e его не покрывает.
>
> Остальные находки «Важно» и «Мелочь» ниже — в очереди, пункт 3 плана в HANDOFF.
