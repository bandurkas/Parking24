...
> **Что уже сделано при закрытии блокеров этапа 1 (22.09, коммиты `b2e3ed7`, `baaf46c`) и где это расходится с разборами ниже.** Код главнее текста ниже: перед этапом 2 сверять с кодом, а не с этим файлом.
>
> - **Обращение без имени** — в коде переменные `{{greeting.hello}}` и `{{greeting.name}}` (а не `client.hello` / `client.dear` из раздела «Шаблоны и рендер»). Без имени `greeting.name` пустая, а первая строчная буква сообщения становится заглавной: «Напоминаем: завтра ждём вас…».
> - **Пустые переменные** — строка выпадает целиком, если все переменные в ней пустые (обращение не считается) и она либо пуста, либо короткая подпись вида «Маршрут:» / «Договор №» без других фраз. Середину строки не режем: сокращения «г.о.», «с.» ломали обрезку по точке.
> - **Трансфер** — `transferLine` и `returnLine` считаются от суток (`FREE_TRANSFER_MIN_DAYS` в `src/lib/tariffs.ts`, только парковка), а не от `Booking.transferNeeded`. Снипеты `line_transfer_yes|no` и `line_return_yes|no`, если их будут делать, выбирать так же, не по галочке.
> - **Цена 0** (фура «по запросу») — новая `{{booking.priceLine}}`: «Стоимость: 1 050 ₽ за 3 суток» или «Стоимость подскажет администратор.»; `dueLine` при цене 0 пустая, чтобы не писать «Бронь оплачена». Суммы выводятся с разбивкой «1 050».
> - **Подтверждение один раз** — `dedupGroup: "confirmation"` у `on_awaiting_payment` и `on_confirmed` уже работает (ключ `confirmation:<id>`), seed на каждом запуске пишет `trigger` и `triggerParams` всех правил. Отменённая запись Outbox ключ не держит — её оживляет следующая постановка (дыра 2 из п. 5 закрыта). `seedBody` не сделан: четыре шаблона помечены `sync`, это надо заменить, когда появится страница правки шаблонов.
> - **`booking_confirmed`** — текст 1 со строкой `{{booking.dueLine}}` вместо «Предоплата не нужна» (а не `payLine`); `awaiting_payment` — текст 1 как в редакции 3.
> - **Тексты шаблонов** лежат в `prisma/templates.ts`, не в `seed.ts`: в рабочий Docker-образ копируется только `prisma/`, импорт из `src/` там не найдётся.
> - **Ссылки** — `siteLinks(tx)` читает настройки в транзакции вызывающего; маршрут по умолчанию `<сайт>/#directions` (такой якорь есть). `NEXT_PUBLIC_SITE_URL` в серверном коде Next не вшивает, если при сборке её нет, и читает во время работы — на stage так и есть, проверено по собранному бандлу.
